...
window.adminViewUserKycDocs = function(uid) {
  ...
  // Fetch full row (includes kyc_url, kyc_back_url, pan_url, aadhaar_number, pan_number etc.)
  getLiveUserFullAsync(uid).then(function(full) { renderKycModal(full || u); }).catch(function() { renderKycModal(u); });
};
